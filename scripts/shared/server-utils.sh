...
# Game validation using global registry
validate_game() {
    local game="$1"
    local env="$2"
    ...
    local global_registry="${REPO_ROOT}/environments/${env}/global-registry.json"
    ...
    if ! command -v jq >/dev/null 2>&1; then
        log_warning "jq not available, skipping game validation"
        return 0
    fi
    
    if jq -r ".supported_games.\"$game\"" "$global_registry" | grep -q "null"; then
        log_error "Game '$game' not supported in $env environment"
        log_info "Supported games: $(jq -r '.supported_games | keys | join(", ")' "$global_registry")"
        return 1
    fi
...
# Instance validation using game-specific registry
validate_instance() {
    local game="$1"
    local instance="$2"
    local env="$3"
    ...
    local game_registry="${REPO_ROOT}/environments/${env}/games/${game}/registry.json"
    
    if [[ ! -f "$game_registry" ]]; then
        log_warning "Game registry not found: $game_registry, creating minimal registry"
        mkdir -p "$(dirname "$game_registry")"
        echo '{"environment": "'$env'", "game": "'$game'", "instances": {}}' > "$game_registry"
        return 0
    fi
    ...
    if jq -r ".instances.\"$instance\"" "$game_registry" | grep -q "null"; then
        log_warning "Instance '$instance' not found in $game registry for $env"
        log_info "Available instances: $(jq -r '.instances | keys | join(", ")' "$game_registry" 2>/dev/null || echo "none")"
        return 1
    fi
...
# Get game information from global registry
get_game_info() {
    local game="$1"
    local env="$2"
    local field="$3"
    ...
    jq -r ".supported_games.\"$game\".\"$field\"" "$global_registry" 2>/dev/null || echo "unknown"
}

# Get instance information from game registry
get_instance_info() {
    local game="$1"
    local instance="$2"
    local env="$3"
    ...
    if [[ ! -f "$game_registry" ]]; then
        log_error "Game registry not found: $game_registry"
        return 1
    fi
    ...
    jq -r ".instances.\"$instance\".\"$field\"" "$game_registry" 2>/dev/null || echo "unknown"
}
...
# Get port assignments from global registry with context offset
get_port_assignments() {
    ...
    if [[ ! -f "$global_registry" ]] || ! command -v jq >/dev/null 2>&1; then
        log_warning "Cannot read port assignments from registry, using defaults"
        if [[ "$env" == "staging" ]]; then
            echo "9215 9019 9577 9999"
        else
            echo "8215 7019 8577 8999"
        fi
        return 0
    fi
    
    # Get base ports for the game
    local game_port_base=$(jq -r ".supported_games.\"$game\".base_ports.game_port" "$global_registry")
    local query_port_base=$(jq -r ".supported_games.\"$game\".base_ports.query_port" "$global_registry")
    local rcon_port_base=$(jq -r ".supported_games.\"$game\".base_ports.rcon_port" "$global_registry")
    local api_port_base=$(jq -r ".supported_games.\"$game\".base_ports.restapi_port" "$global_registry")
    
    # Get context offset from game registry
    local port_offset=0
    local game_registry="${REPO_ROOT}/environments/${env}/games/${game}/registry.json"
    if [[ -f "$game_registry" ]]; then
        port_offset=$(jq -r ".contexts.\"$context\".port_offset // 0" "$game_registry" 2>/dev/null || echo "0")
    fi
    
    # Adjust for environment (staging gets +1000)
    if [[ "$env" == "staging" ]]; then
        game_port_base=$((game_port_base + 1000))
        query_port_base=$((query_port_base + 1000))
        rcon_port_base=$((rcon_port_base + 1000))
        api_port_base=$((api_port_base + 1000))
    fi
    
    local game_port=$((game_port_base + port_offset))
    local query_port=$((query_port_base + port_offset))
    local rcon_port=$((rcon_port_base + port_offset))
    local api_port=$((api_port_base + port_offset))
...
export -f get_volume_name get_container_name get_game_info get_instance_info
